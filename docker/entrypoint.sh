    ...
    if [ -f "server.cfg" ]; then
        ...
        if ! grep -q 'sampvoice' server.cfg 2>/dev/null; then
            if grep -q '^plugins' server.cfg; then
                sed -i 's/^plugins\s*\(.*\)/plugins \1 sampvoice.so/' server.cfg
            else
                printf "plugins sampvoice.so\n" >> server.cfg
            fi
            printf "\033[1;33m[Startup]\033[0m Added sampvoice.so to server.cfg plugins line\n"
        elif grep -q 'sampvoice[^.]' server.cfg 2>/dev/null && ! grep -q 'sampvoice\.so' server.cfg 2>/dev/null; then
            sed -i 's/sampvoice\b/sampvoice.so/g' server.cfg
            printf "\033[1;33m[Startup]\033[0m Fixed sampvoice -> sampvoice.so in server.cfg\n"
        fi
    fi
...
if [ -f "/usr/lib/voicefix.so" ]; then
    ...
    printf "\033[1;36m[VoiceHook] Forcing voice bind to :%s\033[0m\n" "$VOICE_PORT"
else
...
SAMP_PID=$!
sleep 3
if [ -n "$PRELOAD" ] && grep -q 'VoiceHook.*loaded' /tmp/samp.log 2>/dev/null; then
